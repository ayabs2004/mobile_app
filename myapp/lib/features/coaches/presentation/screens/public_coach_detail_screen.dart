...
class PublicCoachDetailScreen extends ConsumerStatefulWidget {
  final String coachId;

  const PublicCoachDetailScreen(
      {super.key, required this.coachId});

  @override
  ConsumerState<PublicCoachDetailScreen> createState() =>
      _PublicCoachDetailScreenState();
}

class _PublicCoachDetailScreenState
    extends ConsumerState<PublicCoachDetailScreen> {
  List<PlayerMediaItem> _mediaItems = [];
  bool _isLoadingMedia = true;
  late final String _scopeKey = 'coach:${widget.coachId}';
  ...
  Widget build(BuildContext context) {
    final coachAsync =
        ref.watch(publicCoachDetailProvider(widget.coachId));
    ...
    return Scaffold(
      ...
      body: coachAsync.when(
        ...
        data: (coach) {
          ...
          final heroImageUrl = coach.coverUrl ?? coach.photoUrl;
          final imageMedia = _mediaItems
              .where((m) =>
                  m.mediaType == 'image' &&
                  m.url != coach.photoUrl &&
                  m.url != coach.coverUrl)
              .toList();
          final videoMedia = _mediaItems
          ...
          return CustomScrollView(
            slivers: [
              ...
              // ── Bloc 1 : infos, certifications, historique, photos ──
              SliverPadding(
                padding:
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (coach.teamHistory.isNotEmpty) ...[
                        ...
                        const SizedBox(height: 10),
                        ...coach.teamHistory.map(
                            (team) => _TeamHistoryTile(
                                team: team)),
                      ],
    ...
    return GestureDetector(
      onTap: () {
        FullscreenPhotoViewer.open(
          ...
          captions:
              allImageMedia.map((m) => m.caption).toList(),
          initialIndex: index,
        );
      },
      child: ClipRRect(
        ...
        child: SizedBox(
          ...
          child: Stack(
            ...
            children: [
              ...
              if (hasCaption)
                _mediaCaptionOverlay(media.caption!,
                    fontSize: 11),
            ],
          ),
        ),
...
class _VideoPlayerCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return _DirectVideoPlayerCard(
        media: media, scopeKey: scopeKey);
  }
}
...
class _EmbedVideoCardState extends State<_EmbedVideoCard> {
  ...
  Widget build(BuildContext context) {
    return VisibilityDetector(
      ...
      onVisibilityChanged: _onVisibilityChanged,
      child: GestureDetector(
        onTap: (_controller == null && !_hasError)
            ? _load
            : null,
        child: Container(
          decoration: BoxDecoration(
